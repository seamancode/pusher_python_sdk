...
class Channel(object):
                ...
                if(not arrArgs.has_key(Channel.USER_ID) or
                   arrArgs[Channel.USER_ID] is None):
                    raise ChannelException(
                        "userId should be specified in optional when pushType is PUSH_TO_USER", self.CHANNEL_SDK_PARAM)
            elif(push_type == Channel.PUSH_TO_TAG):
                if(not arrArgs.has_key(Channel.TAG_NAME) or
                   arrArgs[Channel.TAG_NAME] is None):
                    raise ChannelException(
                        "tagName should be specified in optional when pushType is PUSH_TO_TAG", self.CHANNEL_SDK_PARAM)
    ...
    def _channelExceptionHandler(self, ex):
        print ex.error_msg, ex.error_code
        if self._arrayErrorMap.has_key(ex.error_code):
            print self._arrayErrorMap[ex.error_code]
        else:
            print "self._arrayErrorMap[ex.error_code] not found"
